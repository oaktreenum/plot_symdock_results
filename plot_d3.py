...
# Usage: python plot_d3.py <any_string>
...
def main():
    ...
    tri_scatter_plot = axes[0].scatter(tri_zoom['symmetric_rms'],tri_zoom['I_sc'], c=tri_zoom['Fnat'], cmap="crest", alpha=.8)
    triL_scatter_plot = axes[0].scatter(tri_zoomL['symmetric_rms'],tri_zoomL['I_sc'], marker="+", c="black",alpha=.4, s=60)
    axes[0].set_title('Trimer')
    axes[0].set_xlabel('RMSD')
    xticks = np.arange(0, 20.1, 2.5) 
    axes[0].set_xticks(xticks)
    axes[0].set_ylabel('Interface Score')
    axes[0].annotate(f'PNear: {pnear_tri:.4f}', xy=(0.72, 0.043), xycoords='axes fraction', fontsize=13, color='black')
    cbar0 = plt.colorbar(tri_scatter_plot, ax=axes[0])
    ...
    with open(f"{current_dir}_summary.txt", 'w') as file:
        file.write(f"Trimer Pnear: {pnear_tri}, Pentamer Pnear: {pnear_pent}\n")
...
if __name__ == "__main__":
    if sys.argv[1] == 'help':
        print("""Usage: python plot_d3.py <any_string> 
                    NECESSARY FILE STRUCTURE:
                    RUN_DIRECTORY/
                    ├── 3/
                    │   └── score.sc
